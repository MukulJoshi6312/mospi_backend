    ...
    const { rows } = await query(
      `INSERT INTO banners
         (title, subtitle, banner_type, banner_for, redirect_url,
          start_date, end_date, display_order, status)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
       RETURNING id`,
      [title, subtitle, bannerType, bannerFor, redirectUrl, startDate, endDate, order, status],
    );
  ...
  try {
    ...
    const { rowCount } = await query(
      `UPDATE banners SET
         ...
         updated_at    = NOW()
       WHERE id = $1`,
      [id, title, subtitle, bannerType, bannerFor, redirectUrl, startDate, endDate, order, status],
    );
